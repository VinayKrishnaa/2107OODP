...
public class RoomController {
    ...
    public static final int SIZE_WONDERFUL = 8;
    public static final int SIZE_SPECTACULAR = 7;
    public static final int SIZE_FABULOUS = 4;
    public static final int SIZE_AWAY = 3;
    public static final int SIZE_SUITES = 2;
    ...
    // Attributes which can change: roomStatus, weekdayRate, weekendRate, maxOccupancy, requestButler
    void updateRoomStatus(int roomID, int roomStatus) {
        //roomType to find the right arraylist to access
        int roomNo = roomID % 100;
        switch (getRoomType(roomID)) {
            case 1 -> Wonderful_List.get(roomNo - 1).setRoomStatus(roomStatus);
            case 2 -> Spectacular_List.get(roomNo - 1).setRoomStatus(roomStatus);
            case 3 -> Fabulous_List.get(roomNo - 1).setRoomStatus(roomStatus);
            case 4 -> Away_List.get(roomNo - 1).setRoomStatus(roomStatus);
            case 5 -> Suites_List.get(roomNo - 1).setRoomStatus(roomStatus);
        }
    }

    void updateWeekdayRate(int roomID, float weekdayRate) {
        //roomType to find the right arraylist to access
        int roomNo = roomID % 100;
        switch (getRoomType(roomID)) {
            case 1 -> Wonderful_List.get(roomNo - 1).setWeekdayRate(weekdayRate);
            case 2 -> Spectacular_List.get(roomNo - 1).setWeekdayRate(weekdayRate);
            case 3 -> Fabulous_List.get(roomNo - 1).setWeekdayRate(weekdayRate);
            case 4 -> Away_List.get(roomNo - 1).setWeekdayRate(weekdayRate);
            case 5 -> Suites_List.get(roomNo - 1).setWeekdayRate(weekdayRate);
        }

    }

    void updateWeekendRate(int roomID, float weekendRate) {
        //roomType to find the right arraylist to access
        int roomNo = roomID % 100;
        switch (getRoomType(roomID)) {
            case 1 -> Wonderful_List.get(roomNo - 1).setWeekendRate(weekendRate);
            case 2 -> Spectacular_List.get(roomNo - 1).setWeekendRate(weekendRate);
            case 3 -> Fabulous_List.get(roomNo - 1).setWeekendRate(weekendRate);
            case 4 -> Away_List.get(roomNo - 1).setWeekendRate(weekendRate);
            case 5 -> Suites_List.get(roomNo - 1).setWeekendRate(weekendRate);
        }

    }

    void updateMaxOccupancy(int roomID, int maxOccupancy) {
        //roomType to find the right arraylist to access
        int roomNo = roomID % 100;
        switch (getRoomType(roomID)) {
            case 1 -> Wonderful_List.get(roomNo - 1).setMaxOccupancy(maxOccupancy);
            case 2 -> Spectacular_List.get(roomNo - 1).setMaxOccupancy(maxOccupancy);
            case 3 -> Fabulous_List.get(roomNo - 1).setMaxOccupancy(maxOccupancy);
            case 4 -> Away_List.get(roomNo - 1).setMaxOccupancy(maxOccupancy);
            case 5 -> Suites_List.get(roomNo - 1).setMaxOccupancy(maxOccupancy);
        }
    }

    void updateRequestButler(int roomID, int requestButler){
        int roomNo = roomID % 100;
        switch (getRoomType(roomID)) {
            case 4 -> Away_List.get(roomNo - 1).setMaxOccupancy(requestButler);
            case 5 -> Suites_List.get(roomNo - 1).setMaxOccupancy(requestButler);
            default -> System.out.println("Butler service is not available for this room type.");
        }
    }
